(function () {
    ...
    const s = {
        view: "day",
        anchor: new Date(),
        events: (p.events || []).map(e => ({ ...e, start: new Date(e.start), end: new Date(e.end), services: (e.services || []).map(x => ({ ...x, priceTyiyn: Number(x.priceTyiyn || 0), quantity: Number(x.quantity || 1) })) })),
    ...
    const grid = $("scheduleCalendarGrid"), label = $("schedulePeriodLabel");
    ...
    function setHint(msg) {
        const hint = $("appointmentScheduleHint"); if (!hint) return;
        hint.style.display = msg ? "" : "none"; hint.textContent = msg || "";
    }

    function buildSlots(sc) {
        const box = $("appointmentSlotButtons"); if (!box) return;
        ...
        box.querySelectorAll(".slot-button").forEach(b => b.addEventListener("click", () => { $("appointmentStart").value = b.dataset.start || "09:00"; $("appointmentEnd").value = b.dataset.end || "09:30"; syncScheduleState(); }));
    }
    ...
    function syncScheduleState() {
        ...
        s.scheduleValid = true; setHint(`Смена врача: ${sc.startTime} - ${sc.endTime}.`); if ($("appointmentSaveButton")) $("appointmentSaveButton").disabled = false;
    }

    function renderServices() {
        const tb = $("appointmentServicesTable"); if (!tb) return;
    ...
    function resetForm(slotDate, doctorId) {
        ...
        $("appointmentDoctor").value = doctorId || ""; $("appointmentPaymentType").value = "cash"; $("appointmentStatus").value = "active"; $("appointmentIsActive").checked = true; $("appointmentServiceSelector").value = "";
        const dt = slotDate || new Date(); $("appointmentDate").value = dt.toISOString().slice(0, 10); $("appointmentStart").value = `${dt.getHours().toString().padStart(2, "0")}:${dt.getMinutes().toString().padStart(2, "0")}`;
        const end = new Date(dt); end.setMinutes(end.getMinutes() + 30); $("appointmentEnd").value = `${end.getHours().toString().padStart(2, "0")}:${end.getMinutes().toString().padStart(2, "0")}`;
        s.selectedServices = []; renderServices(); syncScheduleState();
    }
    function fillForm(ev) {
        $("appointmentId").value = ev.id || ""; $("appointmentPatient").value = ev.patientId || ""; $("appointmentPatientId").value = ev.patientId || ""; $("appointmentPatientName").value = ev.patientName || "";
        $("appointmentDoctor").value = ev.doctorId || ""; $("appointmentPhone").value = ev.phone || ""; $("appointmentEmail").value = ev.email || ""; $("appointmentComment").value = ev.notes || "";
        $("appointmentDate").value = ev.start.toISOString().slice(0, 10); $("appointmentStart").value = ev.start.toTimeString().slice(0, 5); $("appointmentEnd").value = ev.end.toTimeString().slice(0, 5);
        $("appointmentReferral").value = ev.referralSource || ""; $("appointmentPaymentType").value = ev.paymentType || "cash"; $("appointmentStatus").value = ev.status || "active"; $("appointmentIsActive").checked = ev.isActive !== false; $("appointmentServiceSelector").value = "";
        ...
        renderServices(); syncScheduleState();
    }
    ...
    function openPeriod(date) {
        s.selectedDate = new Date(date); const list = eventsForDate(s.selectedDate); const pl = $("appointmentPeriodList");
        ...
        if (pl) {
            pl.innerHTML = list.length
                ? list.map(ev => `<button type="button" class="period-appointment-card" data-event-id="${esc(ev.id)}"><div class="period-appointment-card__time">${ft(ev.start)} - ${ft(ev.end)}</div><div class="period-appointment-card__body"><div class="period-appointment-card__title">${esc(ev.patientName || ev.title)}</div><div class="period-appointment-card__meta">${esc(ev.doctor || "Без врача")} · ${esc(ev.status || "active")}</div></div></button>`).join("")
                : '<div class="period-list-empty"><div class="period-list-empty__title">Записей пока нет</div><div class="text-muted small">Вы можете сразу добавить новый прием на выбранную дату.</div></div>';
    ...
    function openDetails(ev) {
        ...
        set("appointmentDetailsPatient", ev.patientName); set("appointmentDetailsPhone", ev.phone); set("appointmentDetailsEmail", ev.email); set("appointmentDetailsDoctor", ev.doctor);
        set("appointmentDetailsDateTime", `${fd(ev.start, { day: "numeric", month: "long", year: "numeric" })}, ${ft(ev.start)} - ${ft(ev.end)}`); set("appointmentDetailsStatus", ev.status); set("appointmentDetailsPaymentType", ev.paymentType); set("appointmentDetailsReferral", ev.referralSource); set("appointmentDetailsNotes", ev.notes);
    ...
    async function generateInvoice() {
        ...
        try {
            ...
            const j = await r.json(); if (!r.ok || !j.success) throw new Error(j.message || "Не удалось сформировать счет.");
    ...
    function renderDay() {
        ...
        grid.querySelectorAll(".schedule-event-chip").forEach(b => b.addEventListener("click", e => { e.stopPropagation(); const ev = s.events.find(x => x.id === b.dataset.eventId); if (ev) openDetails(ev); }));
    ...
    function renderWeek() {
        ...
        const start = new Date(s.anchor); start.setDate(s.anchor.getDate() - ((s.anchor.getDay() + 6) % 7)); const end = new Date(start); end.setDate(start.getDate() + 6);
        label.textContent = `${fd(start, { day: "numeric", month: "long" })} - ${fd(end, { day: "numeric", month: "long", year: "numeric" })}`;
        grid.innerHTML = Array.from({ length: 7 }, (_, i) => { const d = new Date(start); d.setDate(start.getDate() + i); const items = eventsForDate(d); return `<div class="schedule-day-card schedule-day-card--clickable" data-date="${d.toISOString()}"><div class="schedule-day-card__title">${fd(d, { weekday: "short", day: "numeric", month: "short" })}</div>${items.length ? items.map(ev => `<div class="schedule-event-chip schedule-event-chip--static schedule-event-chip--${esc(ev.status)}"><strong>${ft(ev.start)}</strong> ${esc(ev.patientName || ev.title)}</div>`).join("") : '<div class="text-muted small">Нет записей</div>'}</div>`; }).join("");
        grid.querySelectorAll(".schedule-day-card").forEach(c => c.addEventListener("click", () => {
    ...
    function renderMonth() {
        ...
        const ms = monthAnchor(s.anchor), start = new Date(ms); start.setDate(ms.getDate() - ((ms.getDay() + 6) % 7));
        const heads = weekdayNames.map(x => `<div class="schedule-weekday-header">${x}</div>`).join("");
        const cells = Array.from({ length: 35 }, (_, i) => { const d = new Date(start); d.setDate(start.getDate() + i); const items = eventsForDate(d); return `<div class="schedule-month-cell schedule-month-cell--clickable" data-date="${d.toISOString()}"><div class="schedule-month-cell__title">${d.getDate()}</div>${items.slice(0, 3).map(ev => `<div class="schedule-event-chip schedule-event-chip--static schedule-event-chip--${esc(ev.status)}">${ft(ev.start)} ${esc(ev.patientName || ev.title)}</div>`).join("") || '<div class="text-muted small">Нет записей</div>'}</div>`; }).join("");
        grid.innerHTML = heads + cells;
        grid.querySelectorAll(".schedule-month-cell").forEach(c => c.addEventListener("click", () => {
            const d = new Date(c.dataset.date);
            d.setHours(9, 0, 0, 0);
            openPeriod(d);
        }));
    ...
    function render() { if (s.view === "week") return renderWeek(); if (s.view === "month") return renderMonth(); renderDay(); }

    document.querySelectorAll(".schedule-view-switcher [data-view]").forEach(b => b.addEventListener("click", () => {
        s.view = b.dataset.view || "day";
        ...
        if (s.view === "month") s.anchor = monthAnchor(s.anchor); render();
    }));
    document.querySelectorAll(".schedule-nav").forEach(b => b.addEventListener("click", () => {
        ...
        else if (d === "prev") s.anchor = s.view === "month" ? new Date(s.anchor.getFullYear(), s.anchor.getMonth() - 1, 1) : s.view === "week" ? new Date(s.anchor.getFullYear(), s.anchor.getMonth(), s.anchor.getDate() - 7) : new Date(s.anchor.getFullYear(), s.anchor.getMonth(), s.anchor.getDate() - 1);
        else if (d === "next") s.anchor = s.view === "month" ? new Date(s.anchor.getFullYear(), s.anchor.getMonth() + 1, 1) : s.view === "week" ? new Date(s.anchor.getFullYear(), s.anchor.getMonth(), s.anchor.getDate() + 7) : new Date(s.anchor.getFullYear(), s.anchor.getMonth(), s.anchor.getDate() + 1);
        render();
    }));
