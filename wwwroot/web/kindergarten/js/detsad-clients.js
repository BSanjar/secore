...
async function loadChildInfoInto(clientId, contentEl) {
    ...
    try {
        ...
        try {
            const invResp = await fetch(`/Clients/GetInvoicesInfo?clientId=${clientId}`);
            if (invResp.ok) {
...
async function loadPaymentsTab(clientId, contentEl, invoiceId) {
    ...
    try {
        const url = invoiceId 
            ? `/Clients/GetInvoicesInfo?clientId=${clientId}&invoiceId=${encodeURIComponent(invoiceId)}`
            : `/Clients/GetInvoicesInfo?clientId=${clientId}`;
...
async function submitChildAndInvoice() {
    ...
    try {
        ...
        const invoiceRes = await fetch('/Clients/CreateInvoices', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
