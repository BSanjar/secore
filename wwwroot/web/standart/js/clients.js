// JavaScript для страницы "Клиенты" (standart)
document.addEventListener('DOMContentLoaded', function() {
    ...
    childrenCards.querySelectorAll('.child-card--clickable').forEach(function(card) {
        card.addEventListener('click', function() {
            ...
            const clientName = card.getAttribute('data-client-name') || 'Клиент';
    ...
    if (childrenCards) {
        childrenCards.querySelectorAll('.child-row').forEach(function (row) {
            row.addEventListener('click', function (e) {
                ...
                const clientName = row.getAttribute('data-client-name') || 'Клиент';
                if (clientId) openChildDetailModal(clientId, clientName);
            });
        });
    }
    ...
    console.log('Standart clients page loaded');
});
...
function openChildDetailModal(clientId, clientName, onShown) {
    ...
    modalTitle.textContent = clientName || 'Клиент';
...
// Загрузить контент "О клиенте" в указанный элемент
async function loadChildInfoInto(clientId, contentEl) {
    ...
    try {
        ...
        try {
            const invResp = await fetch(`/Invoices/GetInvoicesInfo?clientId=${clientId}`);
            if (invResp.ok) {
            ...
            console.warn('Не удалось загрузить информацию о счёте для карточки клиента', e);
        }
...
async function loadPaymentsTab(clientId, contentEl, invoiceId) {
    ...
    try {
        const url = invoiceId 
            ...
            : `/Invoices/GetInvoicesInfo?clientId=${clientId}`;
        ...
        const downloadPdfUrl = '/Invoices/DownloadPdf?id=' + encodeURIComponent(selectedInvoice.id);
        let html = `
            <div class="details-section">
...
// Функция для показа информации о клиенте (открывает модальное окно с вкладками на первой вкладке)
async function showChildInfo(clientId) {
    openChildDetailModal(clientId, 'Клиент');
}

// Открыть модальное окно на вкладке «Платежи»
function showInvoicesInfo(clientId, invoiceId = null) {
    openChildDetailModal(clientId, 'Клиент', function() {
        const tabPayments = document.getElementById('tab-payments');
        const paymentsContent = document.getElementById('childDetailPaymentsContent');
...
// Функции для модального окна добавления клиента
    ...
    try {
        ...
        if (result.success) {
            ...
            if (clientId && photoInput && photoInput.files && photoInput.files.length) {
                ...
                if (!up.success) {
                    showError((result.message || 'Клиент создан.') + ' ' + (up.message || 'Не удалось загрузить фото.'));
        ...
        return;
            ...
            showError(result.message || 'Ошибка при добавлении клиента');
            submitBtn.disabled = false;
            submitBtn.textContent = 'Зарегистрировать и создать счёт';
            if (btnSkip) btnSkip.disabled = false;
        }
...
async function submitChildAndInvoice() {
    ...
    try {
        ...
        if (!createChildResult.success) {
            showError(createChildResult.message || 'Ошибка при добавлении клиента');
            submitBtn.disabled = false;
            submitBtn.textContent = 'Зарегистрировать и создать счёт';
            if (btnSkip) btnSkip.disabled = false;
            return;
        ...
        if (!clientId) {
            showError('Не получен идентификатор клиента.');
            submitBtn.disabled = false;
            submitBtn.textContent = 'Зарегистрировать и создать счёт';
            if (btnSkip) btnSkip.disabled = false;
            return;
        ...
        if (photoInput && photoInput.files && photoInput.files.length) {
            const up = await uploadChildPhoto(clientId, photoInput.files[0], false);
            if (!up.success) {
                showError('Клиент создан, но фото не загрузилось: ' + (up.message || ''));
                submitBtn.disabled = false;
                submitBtn.textContent = 'Зарегистрировать и создать счёт';
                if (btnSkip) btnSkip.disabled = false;
                return;
        ...
        if (invoiceResult.success !== false) {
            ...
            showError(invoiceResult.message || 'Клиент добавлен, но не удалось создать счёт.');
            submitBtn.disabled = false;
            submitBtn.textContent = 'Зарегистрировать и создать счёт';
            if (btnSkip) btnSkip.disabled = false;
        }
