    ...
    public class WebApiResponseService
    {
        ...
        public static string GetErrorDescription(ErrorCode errorCode)
        {
            return errorCode switch
            {
                ...
                ErrorCode.InvalidTxnDate => "Значение для параметра txn_date имеет неверное значение, формат даты должен быть в виде: yyyyMMddHHmmss, например, (20160101153028)",
                ErrorCode.InvalidAccount => "Неверное значение для параметра account, формат аккаунта должен быть в виде: XXXXXXXXXXX ... (только цифры)",
                ...
                ErrorCode.DuplicateCancelledTxnId => "Была произведена попытка повторной регистрации оплаты на txn_id, который уже был отменен",
                ...
                ErrorCode.InvoiceAlreadyPaid => "Платеж уже оплачен.",
