    ...
    --   admin@kasymov.kg
    --
-- Повторный запуск: пропуск существующих строк (permissions/settings — через WHERE NOT EXISTS).
-- Только пол/дата рождения для рассылки: 2026-07-28_medclinic_patient_demographics_fields.sql
    --
    -- Если ошибка 25P02 («transaction is aborted») или duplicate key:
    --   1) выполните ROLLBACK;
    --   2) запустите скрипт заново целиком (с ROLLBACK; ниже).
    -- =============================================================================
    ...
    SELECT v.id, v.name, v.code, v.description, v.category, v.isdeleted, v.area
    FROM (VALUES
    -- Общие / навигация
    ...
    ) AS v(id, name, code, description, category, isdeleted, area)
    WHERE NOT EXISTS (
        SELECT 1 FROM permissions p WHERE p.id = v.id OR p.code = v.code
    );
    -- ---------------------------------------------------------------------------
    -- 4. Organizations
    -- ---------------------------------------------------------------------------
    ...
    SELECT
        v.organization_id, v.disable_invoice_service_selection, v.allowed_hassameaccount,
        v.invoice_pay_code_mode, v.paymentreminderdaysbefore, v.email, v.whatsapp_phone,
        v.contact_phone, v.director_full_name, v.address, v.logo_path, v.billing_type,
        v.default_qr_mode, v.use_lower_commission_from_org, v.commission_id,
        v.use_upper_commission_from_agent, v.use_lower_commission_to_agent
    FROM (VALUES
    ...
    WHERE NOT EXISTS (
        SELECT 1 FROM organization_settings os WHERE os.organization_id = v.organization_id
    );
    -- ---------------------------------------------------------------------------
    -- 6. Subscriptions
    -- ---------------------------------------------------------------------------
    ...
    -- Medclinic patients (8 шт., у всех заполнены пол и дата рождения)
